...
short_description: A small tool to identify kernel details
description:
  - This module allows you to get current kernel status
...
options:
  lookup_packages:
    ...
    default: true
    description:
      - a flag indicating to check for newer kernel versions
'''

EXAMPLES = '''
- name: "Check kernel version"
  acoby.collection.kernel_info:
    lookup_packages: false
  register: kernel_info

- name: "Reboot for kernel update"
  ansible.builtin.reboot:
    msg: "Kernel changes found, rebooting"
  when: "kernel_info.new_kernel_exists"
'''

RETURN = '''
new_kernel_exists:
  description: Informations about the current kernel
  returned: always
  type: bool
old_packages:
  description: Informations about old kernel packages
  returned: always
  type: str
booted_package:
  description: Informations about the booted kernel packages
  returned: always
  type: str
'''
...
def main():
    ...
    booted_kernel = to_text(
      subprocess.check_output(
        ["grep", "-o", "-P", "(?<=BOOT_IMAGE=).*?(?= )", "/proc/cmdline"]
        ).strip()
      )
    ...
    if params['lookup_packages']:
        for kernel in kernels:
            if kernel.split("/")[-1] == booted_kernel.split("/")[-1]:
                booted_kernel_package = to_text(
                  subprocess.check_output(
                    ["dpkg-query", "-S", kernel]
                  )).split(":")[0]
            elif kernel != latest_kernel:
                old_kernel_packages.append(to_text(
                  subprocess.check_output(
                    ["dpkg-query", "-S", kernel]
                  )).split(":")[0])
